...
var getAltRender = function getAltRender(catalog, id) {
  ...
  lookup = _objectSpread2({
    key: 'altRender',
    altKey: 'render'
  }, lookup);
  var catalogProps = getCatalogProps(catalog, id);
  return jsutils.isObj(catalogProps) && (catalogProps[lookup.key] || catalogProps[lookup.altKey]);
};
...
var Registry = function Registry() {
  ...
  _defineProperty(this, "find", function (cascade, props, _ref) {
    ...
    var type = cascade[0];
    return _this.components[renderKey] || lookup.capitalize && _this.components[jsutils.capitalize(type)] || lookup.type && _this.components[type] || lookup.id && _this.components[cascadeId] || type;
  });
...
var getComponent = function getComponent(cascade, metadata, props, parent) {
  var catalog = metadata.catalog,
      buildCatalog = metadata.buildCatalog;
  var id = props.id;
  ...
  id && buildCatalog && updateCatalogProps(jsutils.eitherObj(catalog[id], {}), props, metadata);
  return FoundComp;
};
...
var renderCascade = function renderCascade(cascade, metadata, parent) {
  ...
  : cascade[0] === metadata.isLoading ? null : jsutils.isArr(cascade)
...
var Cascader = function Cascader(props) {
  ...
  var config = buildConfig(props.config);
  var metadata = {
    catalog: jsutils.isObj(props.catalog) && props.catalog || {},
    styles: props.styles,
    events: props.events,
    config: config,
    isLoading: jsutils.get(config, ['constants', 'CASCADE_LOADING']),
    buildCatalog: props.getCatalog && jsutils.get(config, ['catalog', 'build']) !== false,
    pos: '0'
  };
